...
// ── 自对弈快照 · baseline ──────────────────────────────────────────
// 守擂基准(原名 original.cpp):四份快照中配置最稳健的一版。
// 探索常数使用 DYNAMIC_C_TABLE 多项式拟合查表,渐进扩展阈值最细
// (600/800/1200 → 12/10/8),深层节点采样上限 400。
// 血缘与参数对照见同目录 README.md;与 src/zzmazon_botzone.cpp 相差约 52 行。
    ...
    while(true) {
        ...
        if(!node->is_expanded && (node->visits >= threshold || node == root)) {
            ...
            if(!moves.empty()) {
                ...
                if (node != root) {
                    sampleSize = std::min((int)moves.size(), 400); // 深层节点采样上限 400
                    std::shuffle(moves.begin(), moves.end(), g_rng);
                }
